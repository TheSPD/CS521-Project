...
def fillGrid(photo, grid, colors):
    ...
    for i in range(N-1):
        for j in range(N-1):
            pixel(photo, (i, j), colors[grid[i][j]])

def getIter(i, j, N):
    Q = np.array([[20, 5], [5, 20]], dtype=np.float64)
    return potRed(np.array([[float(i) / N], [float(j) / N]], dtype=np.float64), Q, 0.01)[1]


t0 = time()
# print getIter(1, 1, N)

grid = [[getIter(i, j, N) for i in range(1, N)] for j in range(1, N)]
# print len(grid)
